...
if (!$OutJson) {
  $OutJson = Join-Path $Root 'artifacts/local/bmf-server-save-canary.json'
}
...
$caseRoot = Join-Path $artifactRoot 'bmf-server-save'
...
$saveName = 'BMF_ServerSaveCanary_{0}' -f (Get-Date -Format 'yyyyMMddHHmmss')
...
$describePath = Join-Path $caseRoot 'server-save-describe.json'
$entitiesPath = Join-Path $caseRoot 'server-save-entities.json'
$serverPid = $null
...
try {
  foreach ($path in @($startServerScript, $sendRpcScript, $describeScript, $sourceBmfDir)) {
  ...
  if ($start.verified -ne $true) {
    ...
    Add-Evidence 'brdb' $savedWorldPath 'World BRDB saved through BMF.server.save'

    $describeOutput = & $describeScript -InputPath $savedWorldPath -OutJson $describePath -ParserOutJson $entitiesPath
    ...
    Add-Evidence 'json' $entitiesPath 'Saved world raw parser output'
...
if (Test-Path -LiteralPath $runtimeLogPath) {
  ...
  Add-Evidence 'log' $bmfLogPath 'BMF runtime log with server save evidence'
  $logText = Get-Content -Raw -LiteralPath $bmfLogPath
  foreach ($needle in @(
    'registered console command bmf.server.save',
    'BMF bmf.server.save OK'
  )) {
...
if (Test-Path -LiteralPath $runtimeStatusPath) {
  ...
  Add-Evidence 'json' $statusPath 'BMF runtime status after server save canary'
  try {
...
if (Test-Path -LiteralPath $savedWorldPath) {
  $savedFile = Get-Item -LiteralPath $savedWorldPath
  if ($savedFile.Length -le 0) {
    $errors.Add("Saved world file is empty: $savedWorldPath")
  }
}
...
$archiveData = $null
if ($archiveDescribe) {
  $archiveData = $archiveDescribe.data
}

$result = [ordered]@{
  feature = 'bmf.server.save'
  status = $resultStatus
  ...
  data = [ordered]@{
    ...
    command = @($commandResults | Select-Object -First 1)
    archive = $archiveData
  }
