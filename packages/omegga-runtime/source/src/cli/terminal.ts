...
export default class Terminal {
  ...
  constructor(omegga: Omegga, options: IOmeggaOptions = {}) {
        ...
        if (l.startsWith('DOWNLOADING')) {
          ...
          */
          ...
          try {
            ...
            // ignore redraw races while shutting down
          }
    ...
    omegga.on('control:ready', ({ backend, transport }) => {
      if (backend === 'ue4ss') {
        info(
          'Windows control backend is ready via',
          `UE4SS (${String(transport ?? 'file')})`.yellow,
        );
      }
    });
    omegga.on('control:degraded', ({ detail }) => {
      warn(
        'Windows UE4SS control backend is degraded.',
        'Run',
        'omegga ue4ss validate'.yellow,
        'for diagnostics.',
      );
      if (detail) warn(String(detail));
    });
    ...
    omegga.on('chat', (name, message) => {
      ...
      const label = player?.displayName ?? player?.name ?? name;
      this.log(
        `${label.brightYellow.underline}: ${message}`,
      );
    });
    omegga.on('start', () => {
      const wsl = checkWsl();
      const windowsBackend = this.omegga.getWindowsControlBackend?.();
      const windowsSendKeys = !!process.env.OMEGGA_BRIDGE_SENDKEYS;
      const windowsSendInput = !!process.env.OMEGGA_BRIDGE_SENDINPUT;
      const windowsBridgeVisible = !!process.env.OMEGGA_BRIDGE_KEEP_VISIBLE;
      log(
        `Server has started${
      ...
      if (IS_WINDOWS) {
        if (windowsBackend === 'ue4ss') {
          warn(
            'Windows control is using the managed',
            'UE4SS'.yellow,
            'backend.',
          );
          warn(
            'Run',
            'omegga ue4ss validate'.yellow,
            'to verify signatures, the bridge mod, and command-response parity.',
          );
        } else if (windowsSendKeys) {
          warn(
            'Windows console injection is running in experimental SendKeys mode.',
          );
          if (windowsBridgeVisible) {
            warn(
              'Keep the bridge console visible while testing',
              '/status'.yellow,
              '/cmd'.yellow,
              'and plugins that depend on command responses.',
            );
          } else {
            warn(
              'The bridge may briefly bring its console forward while injecting commands.',
            );
          }
        } else if (windowsSendInput) {
          warn(
            'Windows console injection is running in experimental SendInput mode.',
          );
          if (windowsBridgeVisible) {
            warn(
              'Keep the bridge console focused while testing',
              '/status'.yellow,
              '/cmd'.yellow,
              'and plugins that depend on command responses.',
            );
            ...
            warn(
              'The bridge may briefly steal focus while injecting commands.',
            );
          }
        } else {
          warn(
            'Windows log streaming is working, but the legacy console bridge is still incomplete for the native server.',
          );
          warn(
            'Expect',
            '/status'.yellow,
            '/cmd'.yellow,
            'and plugins that depend on console responses to time out until the Windows console transport is finished.',
          );
        }
      }
  ...
  safePrompt() {
    ...
    try {
      ...
      // ignore prompt races while the terminal is shutting down
    }
  }
  ...
  clearCurrentLine() {
    ...
    try {
      ...
      // ignore terminal redraw races while shutting down
    }
  }
