...
class BmfSocketClient {
  ...
  async connect() {
    await new Promise((resolve, reject) => {
      ...
      socket.on('timeout', () => {
        reject(new Error(`Timed out connecting to BMF socket ${this.meta.host}:${this.meta.port}.`));
        socket.destroy();
      });
...
async function waitForRuntimeSequence(client, sequence, operation, options) {
  ...
  const latestText = latest ? ` latest sequence=${latest.fields.sequence || ''} operation=${latest.fields.operation || ''}` : '';
  throw new Error(`Timed out waiting for runtime operation sequence=${sequence} operation=${operation}.${latestText}`);
}
...
async function queueAndWait(client, command, operation, options) {
  ...
  const completed = await waitForRuntimeSequence(client, sequence, operation, options);
  return { queued, completed };
}

async function queuePhysicalSetAndWait(client, args, commands, visible, collision, operation, options) {
  const deadline = Date.now() + options.timeoutMs;
  let latest = null;
  do {
    ...
    await sleep(Math.min(Math.max(options.pollMs * 4, 500), Math.max(0, deadline - Date.now())));
  } while (Date.now() < deadline);
...
function assertField(errors, fields, key, expected, label) {
  if (String(fields[key] || '') !== String(expected)) {
    errors.push(`${label}: expected ${key}=${expected}, got ${fields[key] || '<missing>'}.`);
  }
}
...
function afterCollisionField(args) {
  return args.guid ? 'item.1.after_collision_channels' : 'after_collision_channels';
}
...
async function main() {
  ...
  const outJson = args.outJson
    ...
    : path.join(root, 'artifacts', 'local', 'bmf-runtime-brick-physical-canary.json');
  ...
  const publicSocketMeta = {
    ...
    tokenPresent: typeof socketMeta.token === 'string' && socketMeta.token.length > 0,
  };
  evidence.push({ kind: 'json', path: socketPath, summary: 'BMF socket metadata used for live canary; token redacted in artifact.' });

  if (!publicSocketMeta.enabled || !publicSocketMeta.started || !publicSocketMeta.port || !publicSocketMeta.tokenPresent) {
    errors.push('BMF socket metadata is not enabled, started, or authenticated.');
  }
  ...
  try {
    if (errors.length === 0) {
      ...
      if (serverStatus.fields.bmf_status !== 'running') {
        errors.push(`Expected bmf_status=running, got ${serverStatus.fields.bmf_status || '<missing>'}.`);
      }
      ...
      if (originalVisible === null || originalCollision === null) {
        errors.push('Initial runtime inspect did not report visible and collision_channels.');
      }
      ...
      assertField(errors, hide.completed.fields, 'ok', 'true', 'hide set result');
      assertField(errors, hide.completed.fields, 'code', 'OK', 'hide set result');
      assertField(errors, hide.completed.fields, afterVisibleField(args), '0', 'hide set result');
      assertField(errors, hide.completed.fields, afterCollisionField(args), '0', 'hide set result');
      ...
      assertField(errors, hiddenInspect.completed.fields, 'visible', '0', 'hidden inspect');
      assertField(errors, hiddenInspect.completed.fields, 'collision_channels', '0', 'hidden inspect');

      if (args.restore && originalVisible !== null && originalCollision !== null) {
        ...
        assertField(errors, restore.completed.fields, 'ok', 'true', 'restore set result');
        assertField(errors, restore.completed.fields, 'code', 'OK', 'restore set result');
        assertField(errors, restore.completed.fields, afterVisibleField(args), String(originalVisible), 'restore set result');
        assertField(errors, restore.completed.fields, afterCollisionField(args), String(originalCollision), 'restore set result');
        ...
        assertField(errors, restoredInspect.completed.fields, 'visible', String(originalVisible), 'restored inspect');
        assertField(errors, restoredInspect.completed.fields, 'collision_channels', String(originalCollision), 'restored inspect');
      }
    }
  } catch (error) {
    errors.push(error && error.message ? error.message : String(error));
  } finally {
    if (client && args.restore && originalVisible !== null && originalCollision !== null) {
      try {
        const visibleArg = originalVisible === 0 ? 'false' : 'true';
