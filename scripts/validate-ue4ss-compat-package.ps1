...
try {
  ...
  if ($packageManifest) {
    ...
    if ([string]$packageManifest.targetBrickadiaBuild -ne 'PC-Shipping-CL15714') {
      $errors.Add('UE4SS compatibility package targetBrickadiaBuild must be PC-Shipping-CL15714.')
    }
  ...
  if ($compatibility) {
    if ([string]$compatibility.brickadia.primaryTarget -notmatch 'PC-Shipping-CL15714') {
      $errors.Add('Compatibility manifest primary target must include PC-Shipping-CL15714.')
    }
  ...
  if (Test-Path -LiteralPath $readmePath) {
    ...
    foreach ($needle in @('UE4SS compatibility component', 'manifests/compatibility.json', 'PC-Shipping-CL15714', 'compat/ue4ss')) {
...
$result = [ordered]@{
  ...
  data = [ordered]@{
    ...
    targetBrickadiaBuild = 'PC-Shipping-CL15714'
  }
