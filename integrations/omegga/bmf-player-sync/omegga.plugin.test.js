...
test('resyncs when Omegga publishes delayed raw player controller data', async t => {
  ...
  const adapter = new BmfPlayerSync(omegga, testConfig(root, { intervalMs: 0 }));
  const reasons = [];
...
test('uses current Brickadia interaction records for exact controller identity', async t => {
  ...
  const cache = JSON.parse(fs.readFileSync(path.join(root, 'players.json'), 'utf8'));
  assert.equal(cache.players.length, 1);
  assert.equal(cache.players[0].playerName, 'Ty');
  assert.equal(cache.players[0].controllerPath, 'BP_PlayerController_C_2147473965');
...
test('writes Omegga bulk positions into the BMF player cache when enabled', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      getPlayers() {
        return [['Ty', 'Ty Display', uuid, 'BP_PlayerController_C_1', 'BP_PlayerState_C_1']];
      },
  ...
  const snapshot = JSON.parse(fs.readFileSync(path.join(root, 'player-positions.json'), 'utf8'));
...
test('writes multiple Omegga bulk positions with distinct player identities', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      getPlayers() {
        return [
          ['Latestore', 'Latestore Display', latestoreId, 'BP_PlayerController_C_1', 'BP_PlayerState_C_1'],
          ['Ty', 'Ty Display', tyId, 'BP_PlayerController_C_2', 'BP_PlayerState_C_2'],
        ];
      },
      async getAllPlayerPositions() {
        return [
  ...
  const positionById = new Map(cache.players.map(player => [player.uuid, player.position]));
  assert.deepEqual(positionById.get(latestoreId), { x: 10, y: 20, z: 30 });
  assert.deepEqual(positionById.get(tyId), { x: 40, y: 50, z: 60 });

  const snapshot = JSON.parse(fs.readFileSync(path.join(root, 'player-positions.json'), 'utf8'));
  assert.equal(snapshot.players.length, 2);
  assert.equal(new Set(snapshot.players.map(record => record.player.id)).size, 2);
  assert.equal(new Set(snapshot.players.map(record => JSON.stringify(record.position))).size, 2);
});
...
test('falls back to targeted control commands for positions when Omegga bulk positions are empty', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      ...
      async execControlCommandWithOutput(command) {
        ...
        if (command === 'GetAll BP_PlayerController_C Pawn Name=BP_PlayerController_C_1') {
          return [
        ...
        if (command === 'GetAll SceneComponent RelativeLocation Name=CollisionCylinder Outer=BP_FigureV2_C_7') {
          return [
  ...
  assert.equal(cache.players.length, 1);
  assert.equal(cache.players[0].uuid, uuid);
  assert.deepEqual(cache.players[0].position, { x: 101.5, y: -202.25, z: 303.75 });
  assert.equal(cache.players[0].positionSource, 'omegga.execControlCommandWithOutput');
  ...
  assert.equal(snapshot.players[0].ok, true);
  assert.equal(snapshot.players[0].player.id, uuid);
  assert.deepEqual(snapshot.players[0].position, { x: 101.5, y: -202.25, z: 303.75 });

  const status = JSON.parse(fs.readFileSync(path.join(root, 'bmf-player-sync-status.json'), 'utf8'));
  assert.equal(status.position.method, 'omegga.execControlCommandWithOutput');
  assert.equal(status.position.fallbackFrom.method, 'omegga.getAllPlayerPositions');
...
test('uses BMF snapshot controller hints when Omegga player records have no controller', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      ...
      async execControlCommandWithOutput(command) {
        commands.push(command);
        if (command === 'GetAll BP_PlayerController_C Pawn Name=BP_PlayerController_C_2147476489') {
          return [
        ...
        if (command === 'GetAll SceneComponent RelativeLocation Name=CollisionCylinder Outer=BP_FigureV2_C_9') {
          return [
  ...
  const cache = JSON.parse(fs.readFileSync(path.join(root, 'players.json'), 'utf8'));
  assert.equal(cache.players[0].controllerPath, 'BP_PlayerController_C_2147476489');
...
test('falls back to BMF player-position snapshot when Omegga position probes are empty', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      getPlayers() {
        return [['Ty', 'Ty Display', uuid, 'BP_PlayerController_C_2147476489', '']];
      },
      async getAllPlayerPositions() {
        return [];
      },
  ...
  assert.equal(cache.players[0].positionSource, 'native.BMFSocketPlayerLocation.fast-cache.pawnAddress');

  const status = JSON.parse(fs.readFileSync(path.join(root, 'bmf-player-sync-status.json'), 'utf8'));
  assert.equal(status.position.method, 'bmf.player-position-snapshot');
  assert.equal(status.position.normalizedCount, 1);
  assert.equal(status.position.fallbackFrom.targeted.method, 'omegga.execControlCommandWithOutput');
...
test('uses only current identified players from BMF all-player position snapshots', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      getPlayers() {
        return [['Latestore', 'Latestore Display', latestoreId, 'BP_PlayerController_C_2147476489', '']];
      },
      async getAllPlayerPositions() {
        return [];
      },
  ...
  assert.deepEqual(cache.players[0].position, { x: 37182.114, y: 44355.323, z: 370.262 });
  assert.equal(cache.players.some(player => player.uuid === staleTyId), false);
...
test('uses OmeggaBridge DescribePlayerLocation when pawn lookup is empty', async t => {
  ...
  const adapter = new BmfPlayerSync(
    {
      getPlayers() {
        return [['Ty', 'Ty', uuid, 'BP_PlayerController_C_1', 'BP_PlayerState_C_1']];
      },
      async getAllPlayerPositions() {
        return [];
      },
  ...
  const cache = JSON.parse(fs.readFileSync(path.join(root, 'players.json'), 'utf8'));
  assert.deepEqual(cache.players[0].position, { x: 111.25, y: -222.5, z: 333.75 });
  assert.match(cache.players[0].positionSource, /^omegga\.bridge\.describe-player-location\./);

  const snapshot = JSON.parse(fs.readFileSync(path.join(root, 'player-positions.json'), 'utf8'));
  assert.deepEqual(snapshot.players[0].position, { x: 111.25, y: -222.5, z: 333.75 });
  const status = JSON.parse(fs.readFileSync(path.join(root, 'bmf-player-sync-status.json'), 'utf8'));
...
test('skips unchanged BMF player cache writes', async t => {
  ...
  assert.equal(adapter.writePlayerCache([player], 'interval-after-external-delete'), true);
  assert.equal(fs.existsSync(cachePath), true);
});
...
test('sends bmf.players.sync over the BMF bridge socket when command bridge mode is enabled', async t => {
  ...
  assert.equal(records[0].pawnPath, 'BP_FigureV2_C_1');
