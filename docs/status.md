...
| Live tested, experimental | Runtime brick state API | `BMF.bricks.inspectRuntimeState` and `BMF.bricks.setRuntimeState` expose explicit runtime brick id visibility/collision inspection and mutation through BMFSocket. The validated path uses Brickadia's native visibility/collision setters with cached, hook-captured, owner-scanned, or background-resolved sparse-grid context. A clean-restart canary hid brick `56357` from `visible=1/collision=255` to `visible=0/collision=0` after a one-time background scan, then restored it to `visible=1/collision=255` from cached context. |
