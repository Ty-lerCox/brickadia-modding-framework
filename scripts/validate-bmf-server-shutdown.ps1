...
if (!$OutJson) {
  $OutJson = Join-Path $Root 'artifacts/local/bmf-server-shutdown-canary.json'
}
...
$caseRoot = Join-Path $artifactRoot 'bmf-server-shutdown'
...
function Wait-ProcessExit([int]$ProcessId, [int]$TimeoutSeconds) {
  ...
  return !(Test-ProcessRunning -ProcessId $ProcessId)
}
...
function Invoke-BmfConsoleCommand(
  ...
  if ($joined -notmatch '^ok=true') {
    $script:errors.Add("BMF command transport did not report ok=true for command: $Command")
  }
...
try {
  ...
  if ($start.verified -ne $true) {
    ...
    Invoke-BmfConsoleCommand 'bmf.server.shutdown' 'bmf-server-shutdown-missing-confirm' @(
      'BMF bmf.server.shutdown CONFIRMATION_REQUIRED',
      'shutdown_scheduled=false',
      'required_confirmation=BMF_SHUTDOWN'
    )

    if (!(Test-ProcessRunning -ProcessId $serverPid)) {
      $errors.Add('Server exited after unconfirmed shutdown command.')
    }

    Invoke-BmfConsoleCommand "bmf.server.shutdown confirm=BMF_SHUTDOWN delayms=$ShutdownDelayMs reason=shutdown-canary" 'bmf-server-shutdown-confirmed' @(
      'BMF bmf.server.shutdown SHUTDOWN_UNAVAILABLE',
      'shutdown_scheduled=false',
      'command=exit',
      "delay_ms=$ShutdownDelayMs",
      'reason=shutdown-canary',
      'api=BMF.server.shutdown',
      'executor=console_manager',
      'executor_code=CONSOLE_EXEC_FAILED'
    )

    $shutdownExited = Wait-ProcessExit -ProcessId $serverPid -TimeoutSeconds 5
  }
} catch {
  $errors.Add($_.Exception.Message)
} finally {
  if ($serverPid -and (Test-ProcessRunning -ProcessId $serverPid)) {
...
if (Test-Path -LiteralPath $runtimeLogPath) {
  ...
  Add-Evidence 'log' $bmfLogPath 'BMF runtime log with server shutdown evidence'
  $logText = Get-Content -Raw -LiteralPath $bmfLogPath
  foreach ($needle in @(
    'registered console command bmf.server.shutdown'
  )) {
...
if (Test-Path -LiteralPath $runtimeStatusPath) {
  ...
  Add-Evidence 'json' $statusPath 'BMF runtime status before confirmed shutdown completed'
} else {
...
if (Test-Path -LiteralPath $runtimeAuditPath) {
  ...
  Add-Evidence 'jsonl' $auditPath 'BMF audit log with server.shutdown record'
  $auditText = Get-Content -Raw -LiteralPath $auditPath
  if ($auditText -notmatch '"action":"server.shutdown"') {
    $errors.Add('Audit log did not include server.shutdown action.')
  }
  if ($auditText -notmatch '"action":"server.shutdown.executed"') {
    $errors.Add('Audit log did not include server.shutdown.executed action.')
  }
} else {
