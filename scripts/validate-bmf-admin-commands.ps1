...
if (!$OutJson) {
  $OutJson = Join-Path $Root 'artifacts/local/bmf-admin-commands-canary.json'
}
...
$caseRoot = Join-Path $artifactRoot 'bmf-admin-commands'
...
$serverPid = $null
$broadcastMessage = '[BMF] Admin command canary {0}' -f (Get-Date -Format 'yyyyMMddHHmmss')
...
try {
  ...
  if ($start.verified -ne $true) {
    ...
    Invoke-BmfConsoleCommand 'bmf.players.list' 'bmf-players-list' @(
      'BMF bmf.players.list OK',
      'players_count=0',
      'adapter=headless-empty'
    )
    Invoke-BmfConsoleCommand "bmf.chat.broadcast message=$broadcastMessage" 'bmf-chat-broadcast' @(
...
if (Test-Path -LiteralPath $runtimeLogPath) {
  ...
  Add-Evidence 'log' $bmfLogPath 'BMF runtime log with admin command evidence'
  $logText = Get-Content -Raw -LiteralPath $bmfLogPath
  foreach ($needle in @(
    'registered console command bmf.chat.broadcast',
    'registered console command bmf.players.list',
...
if (Test-Path -LiteralPath $runtimeStatusPath) {
  ...
  Add-Evidence 'json' $statusPath 'BMF runtime status after admin command canary'
  try {
...
$result = [ordered]@{
  feature = 'bmf.admin-commands.runtime'
  status = $resultStatus
