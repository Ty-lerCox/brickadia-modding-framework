...
| [Plugin raw filesystem](../api/plugins/sandbox.md#environment-containment) | Missing `filesystem.raw` | Lets a plugin open arbitrary filesystem paths for native-helper interoperability. | Use owner-scoped `BMF.storage`; reserve raw files for reviewed bundled integrations. |
