...
After startup, all BMF and OmeggaBridge Lua callbacks run on the game thread.
Keep these UE4SS async helpers unused:

- `ExecuteWithDelay`
- `ExecuteAsync`
- `LoopAsync`

Use one owned, bounded EngineTick one-shot chain where recurring work is
needed. Each delayed link dispatches through the simple EngineTick queue before
registering its successor, so it never mutates the delayed-action container
from inside its traversal. Timers are owner-scoped, cancelled when their plugin
unloads or reloads, and limited by callback-count and elapsed-time budgets per
pump. Missing safe scheduler support is a fail-closed condition, not permission
to fall back to an async Lua callback.
...
`BMFSocket` may perform network I/O on its native worker because that worker
owns native byte queues and does not execute Lua. Only the shared EngineTick
one-shot Lua pump may decode and dispatch received messages.
