...
if (!$OutJson) {
  $OutJson = Join-Path $Root 'artifacts/local/bmf-logging-canary.json'
}
...
$caseRoot = Join-Path $artifactRoot 'bmf-logging'
...
$runtimePluginDir = Join-Path $runtimeBmfDir 'plugins/LoggingCanary'
$runtimeLogPath = Join-Path $runtimeBmfDir 'runtime/bmf.log'
$runtimeEventLogPath = Join-Path $runtimeBmfDir 'runtime/events.jsonl'
$runtimePluginLogPath = Join-Path $runtimeBmfDir 'runtime/logs/plugins/LoggingCanary.log'
...
$pluginStagePath = Join-Path $caseRoot 'logging-plugin-stage.json'
$bmfLogPath = Join-Path $caseRoot 'bmf.log'
$eventLogPath = Join-Path $caseRoot 'events.jsonl'
$pluginLogPath = Join-Path $caseRoot 'LoggingCanary.log'
  ...
  $manifestSource = @'
{
  ...
  "version": "1.0.0",
  "author": "BMF",
  "description": "Temporary logging canary plugin.",
...
'@
  ...
  $pluginSource = @'
local NAME = "LoggingCanary"

return {
  name = NAME,
  onLoad = function(BMF)
    BMF.log("info", "LoggingCanary onLoad", { phase = "load", plugin = NAME })
    BMF.logger.debug("LoggingCanary debug", { phase = "load-debug" })
    BMF.commands.register("bmf.logging.canary", "Logging canary.", function()
      BMF.logInfo("LoggingCanary command", { phase = "command", value = 42 })
      BMF.logWarn("LoggingCanary warning", { phase = "warning" })
      BMF.logError("LoggingCanary error sample", { phase = "error", expected = true })
      return BMF.result(true, "OK", "Logging canary handled", {
        lines = {
          "framework_log=" .. tostring(BMF.logging.frameworkLogPath),
          "event_log=" .. tostring(BMF.logging.eventLogPath),
          "plugin_log_dir=" .. tostring(BMF.logging.pluginLogDir),
          "logging_canary=ok",
        },
      })
    end)
...
'@
  ...
  [ordered]@{
    ...
    command = 'bmf.logging.canary'
  } | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $pluginStagePath -Encoding UTF8
  Add-Evidence 'json' $pluginStagePath 'Temporary LoggingCanary plugin staging result'

  if (Test-Path -LiteralPath $runtimeLogPath) {
    Remove-Item -LiteralPath $runtimeLogPath -Force
  }
  if (Test-Path -LiteralPath $runtimeEventLogPath) {
    Remove-Item -LiteralPath $runtimeEventLogPath -Force
  }
  ...
  if ($start.verified -ne $true) {
    ...
    Invoke-BmfConsoleCommand 'bmf.logging.canary' 'bmf-logging-canary' @(
      'BMF bmf.logging.canary OK',
      'logging_canary=ok',
      'framework_log=ue4ss/main/Mods/BMF/runtime/bmf.log',
      'event_log=ue4ss/main/Mods/BMF/runtime/events.jsonl',
      'plugin_log_dir=ue4ss/main/Mods/BMF/runtime/logs/plugins'
    )
  }
} catch {
...
if (Test-Path -LiteralPath $runtimeLogPath) {
  ...
  Add-Evidence 'log' $bmfLogPath 'BMF framework log with plugin logging evidence'
  $logText = Get-Content -Raw -LiteralPath $bmfLogPath
  foreach ($needle in @(
    '[LoggingCanary] LoggingCanary onLoad',
    '[LoggingCanary] LoggingCanary command',
    '[LoggingCanary] LoggingCanary warning',
    '[LoggingCanary] LoggingCanary error sample',
    'BMF bmf.logging.canary OK'
  )) {
...
if (Test-Path -LiteralPath $runtimePluginLogPath) {
  ...
  Add-Evidence 'log' $pluginLogPath 'LoggingCanary per-plugin log'
  $pluginLogText = Get-Content -Raw -LiteralPath $pluginLogPath
  foreach ($needle in @(
    'LoggingCanary onLoad',
    'LoggingCanary command',
    'LoggingCanary warning',
    'LoggingCanary error sample'
  )) {
    if ($pluginLogText -notmatch [regex]::Escape($needle)) {
      $errors.Add("Plugin log missing expected line: $needle")
    }
  }
} else {
...
if (Test-Path -LiteralPath $runtimeEventLogPath) {
  Copy-Item -LiteralPath $runtimeEventLogPath -Destination $eventLogPath -Force
  Add-Evidence 'jsonl' $eventLogPath 'BMF JSONL event log with plugin records'
  ...
  foreach ($line in [System.IO.File]::ReadLines($eventLogPath)) {
    if ($line.Trim()) {
      try {
        ...
        $errors.Add("Could not parse JSONL event line: $($_.Exception.Message)")
      }
    }
  }
  foreach ($check in @(
    @{ message = 'LoggingCanary onLoad'; phase = 'load' },
    @{ message = 'LoggingCanary command'; phase = 'command' },
    @{ message = 'LoggingCanary warning'; phase = 'warning' },
    @{ message = 'LoggingCanary error sample'; phase = 'error' }
  )) {
    $matches = @($events | Where-Object {
      $_.source -eq 'plugin' -and
      $_.plugin -eq 'LoggingCanary' -and
      $_.message -eq $check.message -and
      $_.data.phase -eq $check.phase
    })
    if ($matches.Count -lt 1) {
      $errors.Add("JSONL events missing plugin event: $($check.message) phase=$($check.phase)")
    }
  }
} else {
  $errors.Add("BMF JSONL event log was not written: $runtimeEventLogPath")
}

if (Test-Path -LiteralPath $runtimeStatusPath) {
  Copy-Item -LiteralPath $runtimeStatusPath -Destination $statusPath -Force
  Add-Evidence 'json' $statusPath 'BMF runtime status after logging canary'
  try {
